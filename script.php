...
# Linux.
$server   = "irc.oldcoder.org";
$port     = 6667;
$nickname = 'GrandbotOld';
$ident    = 'YouShallNotPass';
$gecos    = 'diebot to murder me';
$channel  = "#minetest-general";
function simpletalk ($text)
	{
	global $d, $socket;
...
while (is_resource ($socket))
{
    ...
    if ($d [3] == ":diebot")
    {simpletalk ('We are but bubbles in the foam');
	}
    if ($d [3] == ':@moo')
    {
	...
	$text = "hi" . str_repeat (":)", mt_rand (1, 3));;
	socket_write ($socket, 'PRIVMSG ' . $d[2] . " :$text\r\n"); 
	}
